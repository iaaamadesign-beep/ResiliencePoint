...
(function () {
  ...
  function capOpacity(p, s, e, isIntro) {
    if (isIntro) {
      // the opening line: fully visible from the very start, then fades out
      // on scroll just before the first walkthrough caption begins.
      if (p >= e) return 0;
      var fo = (e - s) * 0.55;
      if (p > e - fo) return clamp((e - p) / fo, 0, 1);
      return 1;
    }
    if (p <= s || p >= e) return 0;
    var fade = (e - s) * 0.24;
  ...
  function frame() {
    ...
    // captions — opacity driven purely by scroll progress (the opening line is
    // just the first caption: visible at p=0, fades out before the next appears).
    for (var i = 0; i < caps.length; i++) {
      var c = caps[i];
      var o = capOpacity(p, c.s, c.e, c.intro);
